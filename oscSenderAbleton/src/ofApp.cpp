...
//	}
   ...
   ofxOscMessage cc;
   cc.setAddress("/of/COHESION");
   float conhesion= panel.getValueF("COHESION_STRENGTH");
   cc.addFloatArg(conhesion);
   sender.sendMessage(cc);
    ...
    ofxOscMessage cr;
    cr.setAddress("/of/COHESION_RADIUS");
    float cradius= panel.getValueF("COHESION_RADIUS");
    float crmapping = ofMap(cradius, 0, 200, 0, 1);
    cr.addFloatArg(crmapping);
    sender.sendMessage(cr);
    
    ofxOscMessage a;
    a.setAddress("/of/ALIGNMENT");
    float alightment = panel.getValueF("ALIGNMENT_STRENGTH");
    a.addFloatArg(alightment);
    sender.sendMessage(a);
 
    ofxOscMessage ar;
    ar.setAddress("/of/ALIGNMENT_RADIUS");
    float aradius= panel.getValueF("ALIGNMENT_RADIUS");
    float armapping = ofMap(aradius, 0, 200, 0, 1);
    ar.addFloatArg(armapping);
    sender.sendMessage(ar);

    
    ofxOscMessage s;
    s.setAddress("/of/SEPERATION");
    float seperation = panel.getValueF("SEPERATION_STRENGTH");
    s.addFloatArg(seperation);
    sender.sendMessage(s);
    
    
    
    ofxOscMessage sr;
    sr.setAddress("/of/SEPERATION_RADIUS");
    float sradius= panel.getValueF("SEPERATION_RADIUS");
    float srmapping = ofMap(sradius, 0, 200, 0, 1);
    sr.addFloatArg(srmapping);
